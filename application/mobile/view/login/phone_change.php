...
<div class="mobile-wrap center">
    ...
    <div class="pers-inform" style="position: relative;top:1.01rem;left:0;">
        <form class="ajax-form" action="{$full_url}" method="post">
            <div>
                <label>新手机号</label>
                <input type="text" name="mobile" id="mobile" placeholder="请输入新手机号">
            </div>
            <div>
                <label>验证码</label>
                <input type="text" value="" placeholder="请输入右侧验证码" name="verify">
                <img src="{:folder_url('Sms/phone2_verify')}" id="verify_img" alt="" class="yzm" style="height: 30px;width: 80px;"/>
            </div>
            <div>
                <label>手机验证码</label>
                <input type="tel" maxlength="6" value="" placeholder="请输入手机验证码" name="code">
                <b>
                    <span id="yzm_phone2" data-wait="0" data-interval="0">获取验证码</span>
                </b>
            </div>
        </form>
    </div>
...
<script>
    $(function () {
        ...
        $('#verify_img').click(function () {
            $(this).attr('src', "{:folder_url('Sms/phone2_verify')}" + '?' + Math.random());
        });

        $('#yzm_phone2').click(function () {
            ...
            $.ajax({
                type: 'POST',
                url: "{:folder_url('Sms/change_mobile2')}",
