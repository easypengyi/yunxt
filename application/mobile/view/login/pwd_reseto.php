        ...
        <div class="login-box">
            <form class="ajax-form" action="{$full_url}" method="post">
                <ul>
                    ...
                    <li>
                        <input type="text" id="verify_pwd" placeholder="请输入右侧验证码" maxlength="5" name="verify">
                        <div class="yzm"><img id="verify_img" src="{:folder_url('Sms/pwd_verify')}" alt=""></div>
                    </li>
                    <li class="tb">
                        <input type="tel" maxlength="6" placeholder="请输入短信验证码" name="code">
                        <span id="yzm_pwd" data-wait="0" data-interval="0">获取验证码</span>
                    </li>
                    <li>
...
<script>
    $(function () {
        ...
        $('#verify_img').click(function () {
            $(this).attr('src', "{:folder_url('Sms/pwd_verify')}" + '?' + Math.random());
        });
    ...
    function sms_count_down(value) {
        set_sms_count_down(value, '#yzm_pwd', function (view, second) {
            ...
            view.text('获取验证码');
            view.attr('style', 'background-color:#e7c875;color:#ffffff;');
        });
    }
