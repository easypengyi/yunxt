 ...
 * 公益报单订单 模块
 */
class Order1 extends AdminController
{
    /**
     * 初始化方法
     * @return void
    ...
    public static  function reward($id){
        ...
        if($order['product_id'] = ProductModel::PRODUCT_ID){
            $money = $order['product_num']*10;
            $member = Member::user_info($order['member_id']);
            $member_id = Member::find_member_city($order['address']['district']);
            if ($member_id){
                Member::commission_inc($member_id, $money);
                MemberCommission::insert_log($member_id, MemberCommission::city, $money, $money, '来自'.$member['member_realname'].'的城市特权奖', 0);
            }
        }
    }
