...
<?php isset($no_check_group_id) OR $no_check_group_id = []; ?>
                    ...
                    <table class="table table-striped table-bordered table-hover" data-href="{$full_url}" data-field="{$order.field}" data-dir="{$order.dir? 1 : 0}">
                        ...
                        <tbody>
                            <?php foreach ($data_list as $v) : ?>
                                <?php $check_group = in_array($v['group_id'], $no_check_group_id); ?>
                                <tr>
                                    <td>{$v.group_name}</td>
                                    <td>
                                        <?php if (!$check_group): ?>
                                            <a class="red open-btn" href="{:controller_url('change_enable')}" data-id="{$v.group_id}" data-status="{$v.enable}">
                                                <div>
                                    ...
                                    <td>
                                        <div class="hidden-sm hidden-xs action-buttons action-buttons-list">
                                            <?php if (!$check_group): ?>
                                                <a href="{:controller_url('access',['id'=>$v.group_id])}" data-toggle="tooltip" title="配置规则">
                                                    <span class="blue"><i class="ace-icon fa fa-cog bigger-130"></i></span>
                                                </a>
                                                <a href="{:controller_url('edit',['id'=>$v.group_id])}" data-toggle="tooltip" title="修改">
                                                    <span class="green"><i class="ace-icon fa fa-pencil bigger-130"></i></span>
                                                </a>
                                                <a href="{:controller_url('del',['id'=>$v.group_id])}" class="confirm-rst-url-btn" data-info="你确定要删除吗？" data-toggle="tooltip" title="删除">
