                    ...
                    <table class="table table-striped table-bordered table-hover" data-href="{$full_url}" data-field="{$order.field}" data-dir="{$order.dir? 1 : 0}">
                        ...
                        <tbody>
                            <?php foreach ($data_list as $v) : ?>
                                <tr>
                                    <td>{$v.name}</td>
                                    <td>{$v.order_shop.order_sn}</td>
                                    <td>{$v.order_shop.product_name}</td>
                                    <td>{$v.order_shop.member.member_nickname}</td>
                                    <td>{$v.order_shop.member.member_tel}</td>
                                    <td>{$v.remark}</td>
                                    <td>{:date('Y-m-d H:i', $v.create_time)}</td>
                                    <td>
                                        <div class="hidden-sm hidden-xs action-buttons action-buttons-list">
                                            <a href="{:controller_url('edit',['id'=>$v.report_id])}" data-toggle="tooltip" title="修改">
