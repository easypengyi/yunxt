                    ...
                    <table class="table table-striped table-bordered table-hover" data-href="{$full_url}" data-field="{$order.field}" data-dir="{$order.dir? 1 : 0}">
                        ...
                        <tbody>
                            <?php foreach ($data_list as $v) : ?>
                                <tr>
                                    <td>{$position[$v.position]}</td>
                                    <td>{$v.name}</td>
                                    <td>{$type[$v.type]}</td>
                                    <td>{$v.sort}</td>
                                    <td>
                                        <a class="red open-btn" href="{:controller_url(['change_enable'])}" data-id="{$v.column_id}" data-status="{$v.enable}">
                                            <div>
                                    ...
                                    <td>
                                        <div class="hidden-sm hidden-xs action-buttons action-buttons-list">
                                            <a href="{:folder_url('ColumnItem/index',['column_id'=>$v.column_id])}" data-toggle="tooltip" title="内容">
                                                <span class="blue"><i class="ace-icon fa fa-list bigger-130"></i></span>
                                            </a>
                                            <a href="{:controller_url('edit',['id'=>$v.column_id])}" data-toggle="tooltip" title="修改">
                                                <span class="green"><i class="ace-icon fa fa-pencil bigger-130"></i></span>
                                            </a>
                                            <a href="{:controller_url('del',['id'=>$v.column_id])}" class="confirm-rst-url-btn" data-info="你确定要删除吗？" data-toggle="tooltip" title="删除">
