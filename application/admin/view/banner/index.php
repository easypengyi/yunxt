                        ...
                        <tbody>
                            <?php foreach ($data_list as $v) : ?>
                                <tr>
                                    ...
                                    <td>{$v.start_time != 0 ? date('Y-m-d H:i',$v.start_time) : ''}</td>
                                    <td>{$v.end_time != 0 ? date('Y-m-d H:i',$v.end_time) : ''}</td>
                                    <td>{$v.sort}</td>
                                    <td>
                                        <a class="red open-btn" href="{:controller_url(['change_enable',['type'=>$type]])}" data-id="{$v.id}" data-status="{$v.enable}">
                                            <div>
                                    ...
                                    <td>
                                        <div class="hidden-sm hidden-xs action-buttons action-buttons-list">
                                            <a href="{:controller_url(['edit',['type'=>$type]],['id'=>$v.id])}" data-toggle="tooltip" title="修改">
                                                <span class="green"><i class="ace-icon fa fa-pencil bigger-130"></i></span>
                                            </a>
                                            <a href="{:controller_url(['del',['type'=>$type]],['id'=>$v.id])}" class="confirm-rst-url-btn" data-info="你确定要删除吗？" data-toggle="tooltip" title="删除">
