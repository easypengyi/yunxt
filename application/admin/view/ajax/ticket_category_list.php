...
<?php foreach ($data_list as $v) : ?>
    <tr tag="{$tag}-{$v.category_id}">
        ...
        <td>{$v.level}级</td>
        <td>
            <a class="red open-btn" href="{:folder_url('TicketCategory/change_enable')}" data-id="{$v.category_id}" data-status="{$v.enable}">
                <div>
        ...
        <td>
            <div class="hidden-sm hidden-xs action-buttons action-buttons-list">
                <a href="{:folder_url('TicketCategory/edit',['id'=>$v.category_id])}" title="修改">
                    <span class="green"><i class="ace-icon fa fa-pencil bigger-130"></i></span>
                </a>
                <a href="{:folder_url('TicketCategory/del',['id'=>$v.category_id])}" class="confirm-rst-url-btn" title="删除" data-info="你确定要删除吗？">
