 ...
 * 商城订单 模型
 */
class OrderssShop extends BaseModel
{
    // 状态
    // 状态-无

    // 状态-待核销
    const STATUS_HEXIAO = 4;
    // 状态-检测中
    ...
    public static function client_list($member_id, $category, $keyword = '')
    {
        ...
        switch ($category) {
            case self::CATEGORY_ALL:
                break;
            case self::STATUS_HEXIAO:
                $where['status'] = self::STATUS_HEXIAO;
                break;
    ...
    {
        return [
            self::STATUS_HEXIAO               => '待核销',
            self::STATUS_CHECKING             => '检测中',
