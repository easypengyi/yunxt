    ...
    <div class="main-content">
        <div class="main-content-inner">
            <div class="page-content">
                <div style="width: 100%;height: 690px;position: relative">
                    <input class="form-control" type="text" id="suggestId" value="" placeholder="请输入地址" style="font-family: '微软雅黑', serif;width: 100%;height: 4%"/>
                    <div id="searchResultPanel" style="border:1px solid #C0C0C0;width:150px;height:auto; display:none;"></div>
                    <div id="allmap" style="font-family: '微软雅黑', serif;width: 100%;height: 92%"></div>
                    <input type="hidden" name="longitude" id="longitude" value=""/>
                    <input type="hidden" name="latitude" id="latitude" value=""/>
                    <input type="hidden" name="address" id="address" value=""/>
                    <input type="hidden" name="edit" id="edit" value="0"/>
                    <button class="btn btn-info" style="width:100%;" id="choose" value="确定">确定</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $('#choose').click(function () {
        if (parseInt($('#edit').val()) === 0) {
            alert('请选择地址！');
            return;
        }

        window.parent['{$callback}']($('#address').val(), $('#longitude').val(), $('#latitude').val());
        parent.layer.closeAll();
        return false;
    });
</script>
<!--suppress JSUnresolvedFunction, JSCheckFunctionSignatures, JSUnresolvedVariable, JSDeprecatedSymbols -->
<script type="text/javascript">
    ...
    AMap.plugin(['AMap.ToolBar', 'AMap.Autocomplete'], function () {
        ...
        AMap.event.addListener(autocomplete, 'select', function (e) {
            ...
            set_info(e.poi.location.lng, e.poi.location.lat, e.poi.name);
        });
    });
        ...
        map.plugin(['AMap.Geocoder'], function () {
            ...
            geocoder.getAddress([longitude, latitude], function (status, result) {
                if (status === 'complete' && result.info === 'OK') {
                    set_info(longitude, latitude, result.regeocode.formattedAddress);
                } else {
                    alert('获取地址失败！')
                }
            });
